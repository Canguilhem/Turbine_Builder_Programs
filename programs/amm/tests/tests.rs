...
use crate::utils::{create_deposit_ix, create_swap_ix, create_withdraw_ix, get_user_atas, token_balance};
...
fn setup()->(
    ...
    // create mints 
    let mint_x= CreateMint::new(&mut svm, &payer).decimals(6).authority(&payer.pubkey()).send().unwrap();
    let mint_y= CreateMint::new(&mut svm, &payer).decimals(6).authority(&payer.pubkey()).send().unwrap();
...
fn test_initialize(){
    ...
    let seed= 123;
    let fee= 30;
    let init =  create_initialize_ix(&mut svm,&payer,mint_x,mint_y,config,seed,fee, mint_lp,vault_x,vault_y);

    let result = send(&mut svm, &[init],&payer,&[&payer]);
    
    let config_account =svm.get_account(&config).unwrap();
    let config_state = amm::state::Config::try_deserialize(&mut config_account. data.as_ref()).unwrap();
    assert!(result.is_ok());
...
fn test_deposit(){
    let (mut svm, payer, mint_x, mint_y, config, mint_lp, vault_x, vault_y) = setup();
    let init =  create_initialize_ix(&mut svm,&payer,mint_x,mint_y,config,123,30, mint_lp,vault_x,vault_y);

    let (user_x, user_y, user_lp) = get_user_atas(&mut svm, &payer, mint_x, mint_y, mint_lp);

    // Mint 1000 X & Y tokens to user ATAs
    MintTo::new(&mut svm, &payer,&mint_x, &user_x, 1_000_000_000).send().unwrap();
    MintTo::new(&mut svm, &payer,&mint_y, &user_y, 1_000_000_000).send().unwrap();
    ...
    let deposit =  create_deposit_ix(
        &mut svm,&payer,mint_x,mint_y,config,mint_lp,
        vault_x,vault_y,user_x,user_y,user_lp,
        deposit_amount, deposit_amount, deposit_amount);
   ...
   assert!(res.is_ok());
    ...
    assert_eq!(vault_x_balance, user_x_balance_before - user_x_balance, "deposit X: should equal balance_before minus balance_after" );
    assert_eq!(vault_y_balance, user_y_balance_before - user_y_balance, "deposit Y: should equal balance_before minus balance_after" );
}
...
fn test_withdraw(){
  ...
  let vault_y_balance= token_balance(&svm, &vault_y);
    ...
    //   As solo LPer
 assert_eq!(user_lp_balance, deposit_amount - withdraw_amount, "current lp should equal deposit minus withdrew amount");
 assert_eq!(vault_x_balance,vault_y_balance,"no swap ie: ratio is still 1:1");
...
fn test_swap(){
    ...
    let swap =  create_swap_ix(
        &mut svm,&payer,mint_x,mint_y,config,mint_lp,
        vault_x,vault_y,user_x,user_y,true, swap_amount, min_amount);
...
assert_eq!(user_x_balance, user_x_balance_before - deposit_amount - swap_amount, "Used X to deposit and swap");

let y_from_swap = user_y_balance
    .saturating_add(deposit_amount)
    .saturating_sub(user_y_balance_before);
assert!(y_from_swap >= min_amount, "should receive at least min Y from swap");


// after: vault -> before - after
assert_eq!(vault_y_balance,user_y_balance_before - user_y_balance, "compare vault Y balance after");
assert_eq!(vault_x_balance,user_x_balance_before - user_x_balance, "compare vault X balance after");
